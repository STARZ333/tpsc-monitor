...
以及**過去 30 天冷門時段參考**（平日/週末 × 2 小時區塊熱力圖，幫你挑人最少的時段）。
...
- `stats.json` 只統計營業時段（06:00–22:00）且期間內確實有人紀錄的時段，
  夜間閉館的恆 0 讀數不會被當成「冷門」；樣本少於 3 筆的區塊會顯示但不列入結論。
  國定假日暫按星期歸類（平日/週末），樣本隨時間累積結論會越來越準。
